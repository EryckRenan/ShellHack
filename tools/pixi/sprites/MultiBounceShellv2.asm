...
;
; Extra Byte 2: determines shell orientation & which palette is used to determine the shell's color, $00-09 = flipped/inverted shell, $0A-0F regular/upright shell.
;
; Extra Byte 3: X speed to spawn the kicked shell with (normal speed is $30).
...
; \ GRABBED THIS SECTION FROM GHOST SHELL BY MarkAlarm
    ...
    LDA !extra_byte_2,x                 ; \ do math on the first extra byte so that the number picked represents its palette number
    SEC : SBC #$08                      ; |
    ASL                                 ; |
    STA $00                             ; /
