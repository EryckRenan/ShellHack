;=======================================================================================;
;                                                                                       ;
; Auto Spin-Jump Shell                                                                  ;
;  By Cracka                                                                            ;
;                                                                                       ;
; A koopa shell which automatically changes depending on players spin-jump type, and    ;
;  switches jump type after interaction. Can be configured as disco as well.            ;
;                                                                                       ;
; Based on Cape Shell by Brakkie, featured in "Baron Of Shell" by shaoshao              ;
;  & Koopa / Shell disassembly + additional features (sprites 4-7, DA-DF) v1.0 by Kevin ;
;                                                                                       ;
;=======================================================================================;
;                                                                                       ;
; Extra Bit:                                                                            ;
;  - If clear, shell palette switches between !NormalSpinPal & !SpinyJumpPal.           ;
;  - If set, rotates through palettes like a traditional disco.                         ;
;                                                                                       ;
;  Note:                                                                                ;
;   Setting the Extra Bit is only useful if Extra Byte 2 disco flag is set.             ;
;                                                                                       ;
;=======================================================================================;
;                                                                                       ;
; Extra Byte 1:                                                                         ;
;  Determines the spawn state of the shell.                                             ;
;   - 00    = Stationary / Carryable                                                    ;
;   - 01    = Kicked (Disco)                                                            ;
;                                                                                       ;
;=======================================================================================;
;                                                                                       ;
; Extra Byte 2:                                                                         ;
;  Determines if shell is disco or not.                                                 ;
;   - 00    = Regular Shell (Not Disco)                                                 ;
;   - 01-FF = Disco Shell                                                               ;
;                                                                                       ;
;  Note:                                                                                ;
;   Normal shell functionality was an afterthought, and setting shell as normal breaks  ;
;    palette if extra bit is set. If it's ever revisited, need to clean that up and     ;
;    switch Extra Bit with Extra Byte 2.                                                ;
;                                                                                       ;
;=======================================================================================;
...
!SpinyJumpPal       = %00000101 ; Palette A (Yellow)
!SpinySpinPal       = %00000111 ; Palette B (Blue)
!NormalJumpPal      = %00000100 ; Palette A (Yellow)
!NormalSpinPal      = %00000110 ; Palette B (Blue)
...
!SpinyTiles         = $0C,$0A,$0E,$08       ; Changed last tile frame from $0C
!ShellTiles         = $8C,$8A,$8E,$8A       ; Changed last tile frame from $8C
...
print "INIT ",pc : Init:
    LDA !extra_byte_2,x
    CMP #$01
    BNE +                   ; Check if we're setting disco flag
    LDA #$01
    STA !187B,x               ; Set Disco flag
+   lda #$01
...
.Carryable:
    LDA !extra_byte_1,x
    BEQ +
    LDA #$0A                ; Spawn as kicked state
    BRA ++
+   LDA #$09                ; Spawn as stationary/carryable state
++  sta !14C8,x             ;/
...
SetCape:
    LDA $140D|!addr             ; $7E140D   Spin Jump flag. #$00 = normal jump (or on ground); any other value = spinjumping.
    BNE +
    LDA #$01
    BRA ++
+   LDA #$00
    BRA ++
;if 00, bit clear, byte 01
;if else, bit set, byte 00
...
+   LDA $140D|!addr         ; $7E140D   Spin Jump flag. #$00 = normal jump (or on ground); any other value = spinjumping.
    BNE +                   ; If normal jump, skip...
    lda #$01                ; ycppccct table, 0 = first gfx page, 1 = second gfx page
    BRA ++
+   LDA #$00
++  CLC : ADC !15F6,x

    LDA !extra_bits,x           ; $7FAB10    If Extra Bit is present...
    AND #$04
    BEQ .NotKicked
