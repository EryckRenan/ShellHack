...
!SpinyStarPal       = %00000101 ; Palette A (Yellow)
!SpinyPSwitchPal    = %00000111 ; Palette B (Blue)
!NormalStarPal      = %00000100 ; Palette A (Yellow)
!NormalPSwitchPal   = %00000110 ; Palette B (Blue)
...
SetCape:
    LDA !extra_byte_1,x
    BNE +
    LDA !extra_byte_2,x     ; $7E1490 Star power timer. Decrements every fourth frame, except when lock flag ($9D) AND while the freezable frame counter ($14) has a value divisible by 4, will decrement each frame (assuming it's a glitch). The music will revert when this timer reaches #$1E.
    STA $1490
    BRA ++
+   LDA !extra_byte_2,x     ; $7E14AD Blue P-Switch timer. Decrements every fourth frame (you can convert seconds to this timer value via PSwitchTimerLength = Seconds * 15). The P-switch running out sound is played when this hits #$1E.
    STA $14AD
    ++    
    LDA #!GiveFly
    BEQ .noFly
