...
;   $00 = Changes on/off switch state to on only.                                       ;
;   $01 = Changes on/off switch state to off only.                                      ;
;   $0A = Changes on/off switch state to the opposite of the current value.             ;
;                                                                                       ;
; Note that if $00 or $01 are used that onced switched to the on/off state, the shells  ;
;  will simply act as a regular shell. Unless this is the intended use, it is           ;
;  recommended that d is used in extra byte 2.                                          ;
;                                                                                       ;
;=======================================================================================;
;                                                                                       ;
; Extra byte 2:                                                                         ;
...
!Regular            = %00001100 ; Palette F
!Flipped            = %10001100 ; Palette F
...
print "INIT ",pc : Init:
    lda !extra_byte_1,x     ;| Set Initial Spawn Orientation
    cmp #$00				; If 00...
    beq +					; Set as regular
    lda #$01				; Otherwise, set as flipped
+   sta !Freeram,x
    lda !KoopaSettings                ;\
...
SetAction:
    ...
    BNE .Flipped                
.Regular
    LDA #$01                ; set as flipped
    STA !Freeram,x
    RTS 
.Flipped
    LDA #$00                ; set as regular
    STA !Freeram,x
    RTS
...
.Orientation
    LDA !Freeram,x
    beq .Regular               ; if clear, set as starting as regular
    cmp #$00
    beq .Regular
    cmp #$01
    beq .Flipped
    bra .Regular               ; catchall, just to be safe... set as regular
