...
; Timer Shell                                                                           ;
;                                                                                       ;  
; Based on Koopa / Shell disassembly + additional features (sprites 4-7, DA-DF)         ;
; v1.0, by KevinM                                                                       ;
;                                                                                       ;
; Interaction Behaviors & Palette Updates                                               ;
; By Cracka                                                                             ;
;                                                                                       ;
; Heavily inspired by Brakkie's Cape Shell from Baron Of Shell by shaoshao              ;
;                                                                                       ;
; Note: Timers lengths can be modified below under Timer Values.                        ;
;                                                                                       ;
;=======================================================================================;
;                                                                                       ;
...
;                                                                                       ;
; Note: Special Shell option only changes value upon initial interaction. Shell will    ;
;  change palette color after initial interaction, then remain that color following the ;
;  act as properties set under !KoopaSettings                                           ;
;                                                                                       ;
;=======================================================================================;
;                                                                                       ;
; Extra Bit:                                                                            ;
...
;                                                                                       ;
; Extra Byte 1:                                                                         ;
;  Sets the action of the shell upon bounce interaction.                                ;
;   $00 = Sets the star power timer.                                                    ;
;   $01 = Sets the star power timer, & prevents star power from killing shell.          ;
;   $02 = Sets the blue p-switch (pow) timer.                                           ;
;   $03 = Sets the flashing invulnerability timer.                                      ;
;                                                                                       ;
;=======================================================================================;
;                                                                                       ;
; Extra byte 2:                                                                         ;
...
; Palettes of the shells (YXPPCCCT)
!StarPal            = %00000100 ; Palette A
!BlueSwitchPal      = %00000110 ; Palette B
!InvulnerabilityPal = %00001100 ; Palette E
!ActiveTimerPal     = %10001100 ; Palette E
...
;=======================================================================================;
;                                                                                       ;
; Original settings for KevinM's extra_byte_1 koopa variables are now set here, because ;
;  Cracka repurposed extra_byte_1 to determine the shell action type.                   ;
;                                                                                       ;
; KevinM's Koopa Settings (Originally Extra Byte 1):                                    ;
...
;   07 (Y): $45                                                                         ;
;                                                                                       ;
; Note: these also matter if spawning the sprite in Shell state, since it can           ;
; become a Koopa if a shell-less Koopa enters it.                                       ;
;                                                                                       ;
;=======================================================================================;

; Koopa Settings
!KoopaSettings      = $40   ; see properties table above
...
; - If far enough onscreen, trigger the block that's been touched.  ;
;                                                                   ;
;===================================================================;
...
SetAction:
    ...
    BEQ .setBluePSwitch
    CMP #$03
    BEQ .setInvulnerability
    RTS
.setStarPower
    LDA #!StarT 			; $7E1490 Star power timer. Decrements every fourth frame, except when lock flag ($9D) AND while the freezable frame counter ($14) has a value divisible by 4, will decrement each frame (assuming it's a glitch). The music will revert when this timer reaches #$1E.
    STA $1490
    BRA +
.setBluePSwitch
    LDA #!BluePSwitchT  	; $7E14AD Blue P-Switch timer. Decrements every fourth frame (you can convert seconds to this timer value via PSwitchTimerLength = Seconds * 15). The P-switch running out sound is played when this hits #$1E.
    STA $14AD
    BRA +
.setInvulnerability
    LDA #!InvulnerabilityT  ; $7E1497 Flashing invulnerability timer - not to be confused with the star timer. This is activated when the player gets hurt. Controls both interaction (player should not interact with sprites again) as well as the blinking graphics.
    STA $1497
+	RTS
...
;    elseif !ColorChange == 1
    ...
    beq .BluePTimerCheck
    cmp #$03
    beq .InvulnerabilityTimerCheck
    bra .DiscoPal
;    endif

.StarTimerCheck
    lda $1490|!addr         ; check if timer is active
    beq +
    lda #!ActiveTimerPal
    bra .DiscoPal
+   lda #!StarPal
	bra .DiscoPal
.BluePTimerCheck
    lda $14AD|!addr         ; check if timer is active
    beq +
    lda #!ActiveTimerPal
    bra .DiscoPal
+   lda #!BlueSwitchPal
	bra .DiscoPal
.InvulnerabilityTimerCheck
    lda $1497|!addr         ; check if timer is active
    beq +
    lda #!ActiveTimerPal
    bra .DiscoPal
+   lda #!InvulnerabilityPal
	bra .DiscoPal
...
;    asl                     ;|
;    tax                     ;|
;    lda KoopaTilemap,x          ;| Store tile numbers to OAM.
;    sta $0302|!addr,y       ;|
;    lda KoopaTilemap+1,x        ;|
;    sta $0306|!addr,y       ;/
;    ldx $15E9|!addr
;    lda $01                 ;\
;    sta $0301|!addr,y       ;|
