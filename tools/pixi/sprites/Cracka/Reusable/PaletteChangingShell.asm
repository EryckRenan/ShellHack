...
;                                                                                       ;
; Note: Freeram required, defines set below under !Freeram                              ;
;                                                                                       ;
;=======================================================================================;
;                                                                                       ;
...
;=======================================================================================;
;                                                                                       ;
; Extra Byte 1 Is not currently utilized for this sprite                                ;
;                                                                                       ;
;=======================================================================================;
;                                                                                       ;
; Extra byte 2:                                                                         ;
...
!Freeram			= $7E0058	; Ensure this isn't already used elsewhere in the level

; Palettes of the shells (YXPPCCCT)
!Palette8           = %00000000 ; Palette 8
!Palette9           = %00000010 ; Palette 9
!PaletteA           = %00000100 ; Palette A (Yellow)
!PaletteB           = %00000110 ; Palette B (Blue)
!PaletteC           = %00001000 ; Palette C (Red)
!PaletteD           = %00001010 ; Palette D (Green)
!PaletteE           = %00001100 ; Palette E
!PaletteF           = %00001110 ; Palette F
...
;	BEQ +
;	CMP #$09
;	BCC +					; If 08 or 09 Set Act As Green
;;	CMP #$0A
;	BEQ ++
;	CMP #$0B
;	BEQ +++
;	CMP #$0C
;	BEQ ++++
;							; If 0D, 0E, or 0F Set Act As Green
;+	LDA #$40				; Act As Green Shell
;	BRA +
;++	LDA #$45				; Act As Yellow Shell
;	BRA +
+++	LDA #$43				; Act AS Blue Shell
;	BRA +
;++++ LDA #$42				; Act AS Red Shell
...
SetAction:
    LDA !Freeram,x
    BEQ .SetInitialPalette  ; if clear, set as 0E
    CMP #$0F                
    BEQ .ResetPalette       ; if 0F, set as 08
    INC #$01                ; increase value by 01
    STA !Freeram,x
    RTS
.SetInitialPalette
    LDA #$09                ; set as palette 09, after first interaction
    STA !Freeram,x
    RTS 
.ResetPalette
    LDA #$08                ; set as palette 08
    STA !Freeram,x
    RTS
...
.PaletteCheck
    LDA !Freeram,x
    beq .Pal8               ; if clear, set as starting palette 08
    cmp #$08
    beq .Pal8
    cmp #$09
    beq .Pal9
    ...
    cmp #$0F
    beq .PalF
    bra .PalD               ; catchall, just to be safe... set as 08
