...
!value = $D8		 ; Address of the value you want to display
!indexed = 1		 ; 1 = address is X indexed like a sprite table, set the index as below
...
!indexsprite = #$09  ; If not FF, will search for a sprite matching this number and use that index ($9E)


!mode = 3; 0 = text only, 1 = horizontal meter, 2 = vertical meter, 3 = horiz+text, 4 = vert+text

!abs = 1			 ; 1 = treat negative values as positive (absolute value)
...
!TextY = #$2C		; Y Position of text value, in pixels
...
!BarSize = #$05		; How many tiles long the bar is (8x8)
!BarTile = #$80		; Base tile for full bar, expects tiles for partial bars to follow
!BarWide    = 1		; Makes a bar two tiles wide/tall
!BarWideDbl = 1		; If 1, the bar tile is flipped and draw twice to fill the double bar, otherwise it is just centered in the wide bar

!BarX = #$1C		; X Position of bar, in pixels
!BarY = #$28		; Y Position of bar, in pixels
!BarProp = #$35		; Properties to use for bar tiles

!BarOverlay = 1		; If 1, draws a static overlay graphic for the bar to fill
!BarOverL = #$98	; The left or top end of the overlay graphic, must be two wide/tall for a wide bar
!BarOverM = #$99	; The middle of the overlay graphic which is repeated to fill length of bar, must be two wide/tall for a wide bar
!BarOverR = #$9A	; The right or bottom end of the overlay graphic, must be two wide/tall for a wide bar
!BarOverProp = #$33 ; Properties to use for overlay tiles

!BarBG     = 0		; If 1, will still draw tiles to fill empty space in an unfilled bar
